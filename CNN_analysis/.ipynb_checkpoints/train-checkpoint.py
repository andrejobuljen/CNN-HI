...
# data parameters
root_out        = '/shares/stadel.ics.mnf.uzh/aobulj/hifi_mocks_Paco_noise/cnn_full_noise_smooth2D/' #output folder
...
fields          = ['fiducial']  #repeat Mgas, HI, ne, MgFe
params          = [0,1,2,3,4,5] #0(Om) 1(s8) 2(b_1) 3(b_2) 4(b_G2) 5(Psn)
...
f_maps, f_maps_norm, suffix = U.fname_maps(root_maps, sim, z)
print (f_maps)
storage_m = U.fname_storage(root_storage, label, monopole=monopole)
...
study = optuna.create_study(study_name=study_name, sampler=sampler, storage=storage_m, load_if_exists=True)
