...
#     for field in fields:
#         suffix = '%s_%s'%(suffix,field)
#         f_maps.append('%s/Maps_%s_%s_%s_z=%.2f.npy'%(root,field,sim,suite,z))
#         if sim_norm is None:
#             if suite=='LH':
#                 f_maps_norm.append(None)
#             else:
#                 f_maps_norm.append('%s/Maps_%s_%s_LH_z=%.2f.npy'%(root,field,sim,z))
#         else:
#             f_maps_norm.append('%s/Maps_%s_%s_LH_z=%.2f.npy'%(root,field,sim_norm,z))

#     return f_maps, f_maps_norm, suffix

# def fname_maps(root, sim, z, sim_norm=None):
#     f_maps, f_maps_norm, suffix = [], [], ''
#     f_maps.append('%s/maps_100.npy'%(root))
#     f_maps_norm.append(None)
#     return f_maps, f_maps_norm, suffix
    
def fname_maps(root, sim, z, sim_norm=None):
    f_maps, f_maps_norm, suffix = [], [], ''
    f_maps.append('%s/maps_1000_kmax2D_0.20.npy'%(root))
    f_maps_norm.append(None)
    return f_maps, f_maps_norm, suffix

# This routine returns the name of the storage databases
def fname_storage(root, label_m, label_e=None, monopole=True):
