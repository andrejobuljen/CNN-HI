...
# load Sobol sequence:
sobol = np.load('sobol_sequence_om_s8_b1_b2_bG2_Psn.npy')
Om, s8, b1, b2, bG2, Psn = sobol[seed] 
...
print ("using Omega_M = %.3f, sigma8=%.3f, b1=%.3f, b2=%.3f, bG2=%.3f, Psn=%.2f ..."\
       %(Om, s8, b1, b2, bG2, Psn))

# Cosmology
c = cosmology.Cosmology(h=0.6774, T0_cmb=2.725, Omega0_b=0.0486, Omega0_cdm=Om-0.0486, m_ncdm=[], n_s=0.9667, k_pivot=0.05, A_s=2.055e-9, YHe=0.24)
c = c.match(sigma8=s8)
...
HI_field_poly = polynomial_field_cnn(d1, d2, dG2, d3, params_path, zout, p1, b1, b2, bG2)
print ('done (elapsed time: %1.f sec.)'%(time.time()-start))

# Add isotropic stochastic noise:
print ('Adding noise... ')
HI_field_poly += whitenoise(Psn, Nmesh, BoxSize)
print ('done (elapsed time: %1.f sec.)'%(time.time()-start))
...
f = h5py.File(out_fname + '.hdf5','w')
f.create_dataset('delta_HI',  data=FieldMesh(HI_field_poly).to_field())
f.close()
print ('done (elapsed time: %1.f sec.)'%(time.time()-start))

# load and measure Pk
f = h5py.File(out_fname + '.hdf5', 'r')
HI_field_poly = f['delta_HI'][:]
f.close()
print ('loaded delta HI numbers: ', np.min(HI_field_poly), np.max(HI_field_poly), np.mean(HI_field_poly))
HI_field_poly = ArrayMesh(HI_field_poly, BoxSize)
...
if plot:
	...
	# plt.loglog(pHI.power.coords['k'], pHI_bfile.power['power'].real, '.:', label = 'HI mock bfile')
	...
	plt.xlim(right=1)
	plt.ylim(1e1,5e4)
	plt.axhline(Psn, ls='-', color='k', label = 'input Psn')
	plt.axhline((BoxSize/Nmesh)**3, ls=':', color='gray', label = 'expected noise')
	plt.legend(loc=0)
